...
import com.cherryfunding.spring.util.KakaoApi;
...
//		vo = service.kakaoLogin(vo);
		...
		// access token 지정
		apiHelper.setAccessToken(token.path("access_token").toString());

		// 푸시 알림이나 유저 아이디 리스트가 필요할 때 설정 합니다.
		// (디벨로퍼스 내에 앱설정 메뉴를 가시면 있습니다)
		apiHelper.setAdminKey(adminKey);
	...
	public void testUserManagement() {
		...
		System.out.println(apiHelper.me());
	...
	public void testKakaoStory() {
		...
		/*
		 * // 복수개의 내스토리 정보 요청 // https://dev.kakao.com/docs/restapi#복수개의-내스토리-정보-요청
		 * apiHelper.getMyStories();
		 */

		/*
		 * // 복수개의 내스토리 정보 요청 (특정 아이디 부터) paramMap = new HashMap<String, String>();
		 * paramMap.put("last_id", TEST_MYSTORY_ID); apiHelper.getMyStories(paramMap);
		 */
		 ...
		 * result.indexOf("code\":-") == -1) { String postedId = result.split("\"")[3];
		 * System.out.println("postedId:" + postedId); if
		 * (apiHelper.deleteMyStory(postedId).equals(""))
		 * System.out.println("deleted test my story " + postedId); }
		 */

		/*
		 * // 사진 포스팅 (최대 10개까지 가능) String uploadedImageObj = apiHelper.uploadMulti( new
		 * File[]{ TEST_UPLOAD_FILE1, TEST_UPLOAD_FILE2 }); if (uploadedImageObj !=
		 * null) { System.out.println("uploaded file(s) successfully.");
		 * System.out.println(uploadedImageObj); paramMap.put("image_url_list",
		 * uploadedImageObj); result = apiHelper.postPhoto(paramMap); if (result != null
		...
		/*
		 * // 링크 포스팅 final String linkInfoObj = apiHelper.getLinkInfo(TEST_SCRAP_URL);
		 * if (linkInfoObj != null) { paramMap.put("link_info", linkInfoObj); result =
