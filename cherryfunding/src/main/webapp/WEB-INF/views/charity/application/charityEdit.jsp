...
<div class="slideshow-container">
	...
	<c:forEach var="vo" items="${cPictureList}" varStatus="vs">
		<div class="mySlides fade0" <c:if test="${vs.first}">style="display:block;"</c:if>>
		    <div class="numbertext">${vs.count} / ${fn:length(cPictureList)}</div>
		    <img src="${vo.saveName}" style="width:100%">
			<div class="text">${vo.cpinfo}</div>
  		</div>
	</c:forEach>
...
<div id="summernote">${vo.content}</div>
	...
	var save = function() {
		...
		$.ajax({
			url:'${pageContext.request.contextPath}/charity/editContent',
			data:{cNum:'${vo.cNum}', content:markup},
			dataType:'json',
			type:'post',
